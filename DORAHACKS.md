...
**Vision** (202/256 characters)

IdemGate proves business outcomes do not happen twice: 100 duplicate Stripe events → 1 charge, 99 blocked → CloudWatch metric 99 → signed S3 evidence → GO. Any failed or missing proof blocks production.
...
IdemGate is a proof-gated master prompt for production AWS payment, order, subscription, deployment, and webhook workflows. It forces an AI coding agent to connect every attack to a named AWS control, numeric business result, exact CloudWatch metric, raw receipt, and immutable evidence bundle. Example: 100 identical Stripe events → DynamoDB `TransactWriteItems` → 1 charge + 99 blocked → `DuplicateBlockedCount=99` → signed S3 Object Lock bundle → `GO`. Other prompts build event pipelines. IdemGate proves the business outcome did not happen twice.
...
**Example Output** (658/960 characters)

PROOF GATE RESULTS — RECORDED, NOT CLAIMED. Duplicate Storm: 100x Stripe checkout.session.completed → DynamoDB TransactWriteItems → 1 charge + 99 blocked → DuplicateBlockedCount=99 → PASS. Race: 50 workers → 1 winner + 49 blocked → PASS. Replay: signed webhook, 48h late → 0 queue messages → ReplayRejectedCount=1 → PASS. Invariant safety: 0 intents >1; max=1. Invariant liveness: 3 accepted-success intents → 3 exactly-one outcomes; lost=0. VERDICT: GO. Evidence: 8 raw receipts + canonical JSON/Markdown, SHA-256 and KMS signature verified, S3 Object Lock COMPLIANCE. Path: s3://idemgate-evidence-ACCOUNT-us-east-1/runs/RUN_ID/idemgate-evidence-bundle.json
